...
contract RouletteLogicInternal {
    ...
    function launch(bytes32 _localhash) external returns(uint256 winAmount, uint256 number) {
    ...
    function getPayoutForType(BetType betType) public pure returns(uint256) {
        if (betType == BetType.Single) return 36;
...
// File: contracts/roulette-example/HashChain.sol
