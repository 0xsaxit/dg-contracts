...
contract SlotMachineMANA is HashChain, AccessControl {
    ...
    event SMLChanged(SlotMachineLogicOld indexed sml);
    ...
    SlotMachineLogicOld public sml;

    constructor(SlotMachineLogicOld _sml) public {
        sml = _sml;
        emit SMLChanged(sml);
    }

    function changeSML(SlotMachineLogicOld _sml) public onlyCEO {
        sml = _sml;
        emit SMLChanged(sml);
    }
    ...
    function tes(bytes32 _localhash) external onlyCEO returns (bool) {
        _consume(_localhash);
        return true;
    }
